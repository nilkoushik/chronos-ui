...
* **effects:** allow injecting a custom background-effect plugin ([a22984c](https://github.com/nilkoushik/chronos-ui/commit/a22984c67325e2d6556c6538b889400400c1dafa))
...
- **Pluggable background-effect engine.** Banner, SlidingBanner, and TimerWidget accept an optional `backgroundEffectPlugin` (`config.backgroundEffectPlugin` for Banner/SlidingBanner, a top-level prop for TimerWidget) — a `{ start, stop }` pair letting a consumer (e.g. a CMS page editor) swap in its own canvas animation implementation per-instance. Import `defaultBackgroundEffectPlugin` from `src/utils/backgroundEffects.ts` (or `@chronos-ui/core/{react,svelte}/utils/backgroundEffects`) to reuse or wrap the built-in effects; omitting the prop keeps existing behavior unchanged.
