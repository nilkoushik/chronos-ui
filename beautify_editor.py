...
export default function RichTextEditor(props: RichTextEditorProps) {
  ...
  return (
    ...
    >
      ...
      <div class="editor-toolbar flex flex-wrap gap-x-4 gap-y-3 p-3 select-none sticky top-0 z-10 w-full backdrop-blur-md" 
           style={{ 
        ...
        <div class="flex items-center gap-1 text-slate-300 bg-black/20 p-1 rounded-lg border border-white/5">
          <button type="button" class="font-bold text-sm w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('bold')} title="Bold">B</button>
          <button type="button" class="italic text-sm w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 hover:text-white transition-colors font-serif" onClick={() => state.format('italic')} title="Italic">I</button>
          <button type="button" class="underline text-sm w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('underline')} title="Underline">U</button>
          <button type="button" class="line-through text-sm w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('strikeThrough')} title="Strikethrough">T</button>
        </div>

        <div class="w-px h-6 bg-white/10"></div>

        <div class="flex items-center gap-1 text-slate-300">
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.formatHeading('PRE')} title="Code Block">
          ...
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.formatHeading('BLOCKQUOTE')} title="Blockquote">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z"/><path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z"/></svg>
          </button>
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('removeFormat')} title="Clear Formatting">
        ...
        <select class="bg-black/20 border border-white/10 text-slate-300 font-semibold text-xs rounded-lg px-3 py-1.5 outline-none focus:border-violet-500 transition-colors" onChange={(e) => state.formatHeading(e.target.value)}>
        ...
        <div class="flex items-center gap-1 text-slate-300">
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('justifyLeft')} title="Align Left">
          ...
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('justifyCenter')} title="Align Center">
          ...
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('justifyRight')} title="Align Right">
        ...
        <div class="flex items-center gap-1 text-slate-300">
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.insertMedia('image')} title="Image">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
          </button>
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.openLinkModal()} title="Link">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
          </button>
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.openTableModal()} title="Table">
          ...
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('insertUnorderedList')} title="Bullet List">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
          </button>
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.format('insertHorizontalRule')} title="Horizontal Line">
          ...
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.insertMedia('video')} title="Video">
        ...
        <div class="flex items-center gap-2">
          <button type="button" class="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-slate-300 font-semibold text-xs hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.insertButton()}>Insert Button</button>
          
          <button type="button" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-xs transition-all duration-200 bg-pink-500/10 text-pink-400 border border-pink-500/30 hover:bg-pink-500/20" onClick={() => state.openWidgetModal()}>
        ...
        <div class="flex items-center gap-1 text-slate-400">
          <button type="button" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 hover:text-white transition-colors" onClick={() => state.syncContent()} title="Save">
