...
> **No longer published.** `1.4.2` is the last version of this wrapper, and
> releases no longer mirror into it. The rename has landed and consumers have
> had a forwarding version to migrate from, so every release after `1.4.2`
> ships under the new name only. `1.4.2` stays on npm and keeps forwarding to
> `@contentvidya/ui@1.4.2`, so existing installs continue to resolve — they
> just will not receive new versions. Migrate to `@contentvidya/ui` to get
> anything newer.
>
